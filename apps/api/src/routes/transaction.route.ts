...
transactionRouter.patch('/:id', async (req, res, next) => {
  try {
    const updated = await transactionService.update(req.params.id, req.user!.id, req.body);
    res.json(updated);
  } catch (err: any) {
    if (err.statusCode) {
      return res.status(err.statusCode).json({ error: err.message });
...
transactionRouter.delete('/:id', async (req, res, next) => {
  try {
    ...
    if (!deleted) {
      return res.status(404).json({ error: 'Transaction not found or could not be reverted' });
    }
