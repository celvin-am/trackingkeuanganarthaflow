  ...
  return (
    <header className="sticky top-0 h-20 bg-neutral-50/80 backdrop-blur-xl flex items-center justify-between px-10 z-40 border-b border-neutral-100/50">
      <div className="flex flex-col">
        <h2 className="text-xl font-bold tracking-tight text-on-surface">{getGreeting()}, {session?.user?.name?.split(' ')[0] || 'User'} 👋</h2>
        <p className="text-xs text-secondary font-medium">Your financial overview is looking healthy today.</p>
      </div>
      <div className="flex items-center gap-6">
        ...
        <div className="relative group cursor-pointer">
          ...
          <div className="absolute right-0 top-full mt-2 w-32 bg-white rounded-xl shadow-xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 border border-neutral-100 z-50">
            <button onClick={() => setCurrency('IDR')} className={`w-full text-left px-4 py-2 hover:bg-neutral-50 text-sm font-semibold ${currency === 'IDR' ? 'text-primary' : 'text-on-surface'}`}>IDR (Rp)</button>
            <button onClick={() => setCurrency('USD')} className={`w-full text-left px-4 py-2 hover:bg-neutral-50 text-sm font-semibold ${currency === 'USD' ? 'text-primary' : 'text-on-surface'}`}>USD ($)</button>
            <button onClick={() => setCurrency('EUR')} className={`w-full text-left px-4 py-2 hover:bg-neutral-50 text-sm font-semibold ${currency === 'EUR' ? 'text-primary' : 'text-on-surface'}`}>EUR (€)</button>
          </div>
        </div>
        {/* Notifications */}
        <div className="relative group cursor-pointer">
          ...
          <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-xl shadow-xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 border border-neutral-100 z-50">
            ...
            <div className="max-h-64 overflow-y-auto">
              <div className="px-4 py-3 hover:bg-neutral-50 cursor-pointer flex gap-3">
                ...
                <div>
                  <p className="text-sm text-on-surface font-semibold">Welcome to ArthaFlow Finance!</p>
                  <p className="text-xs text-secondary mt-0.5">Let's record your first transaction.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="relative group cursor-pointer">
          ...
          <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 border border-neutral-100 z-50">
            <div className="px-4 py-3 border-b border-neutral-100 mb-2">
              <p className="text-sm font-bold text-on-surface truncate">{session?.user?.name || 'User'}</p>
              <p className="text-xs text-secondary truncate">{session?.user?.email || 'user@example.com'}</p>
            </div>
            <button onClick={() => navigate('/settings')} className="w-full text-left px-4 py-2 hover:bg-neutral-50 text-sm font-semibold text-on-surface flex items-center gap-3">
              <span className="material-symbols-outlined text-[18px]">settings</span>
              Settings
            </button>
            <button onClick={handleLogout} className="w-full text-left px-4 py-2 hover:bg-neutral-50 text-sm font-semibold text-red-600 flex items-center gap-3 mt-1 border-t border-neutral-100 pt-3">
